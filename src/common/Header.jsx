...
export default function Header() {
  ...
  return (
    <>
        ...
        <div className="max-w-[1170px] mx-auto items-center flex justify-between">
          ...
          <div className="py-[15px]">
            <ul className="flex flex-col sm:flex-row items-center poppins-regular text-[#32343b]">
              {myLink.map((v, i) => {
                return (
                  <li key={i} className="mr-[60px] relative group">
                    <Link
                      to={v.to}
      ...
      {/* Mobile Header */}
      <header className="w-full shadow-2xl bg-white sticky top-0 left-0 sm:hidden block z-[999] ">
        ...
        >
          <ul className="flex flex-col max-h-[200px] mt-20 space-y-0 px-6 poppins-regular text-[#32343b]">
            {myLink.map((v, i) => (
              <li key={i} className="group py-1 text-[15px]">
                <Link to={v.to} className="hover:text-[#be8553] font-medium">
                  {v.firstkey}
                </Link>
          ...
          <div className="border-t border-gray-300 mt-8 pt-4 px-6">
            <p className="capitalize text-blue-500 font-semibold mb-4">
              Social Connect
            </p>
      ...
      {/* Enquiry Button */}
      <div className="fixed top-[50%] translate-x-[-50%] z-50 text-white lg:right-[-80px] right-[-60px] bg-[#be8553] rotate-90 px-[20px] py-[10px]">
